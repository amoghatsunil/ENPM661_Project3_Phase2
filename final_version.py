...
scale = 200 # 200 pixels = 1 meter
h=0
clear_process=clear*scale/1000 #clearance for 2D visualization from obstacles
actual_width = 5.4
actual_height= 3
...
# === Function to get and validate start and goal positions ===
...
# === Inflate obstacles by clearance to get clearance zones ===
...
# === Differential drive cost function with collision check ===
def cost(Xi, Yi, Thetai, UL, UR, clearance_map):
    ...
    while t < 1: #updates x,y,theta for 0.1 s increment and updates D which is then used as cost to come
        t += dt
        Delta_Xn = 0.5 * r * (UL + UR) * math.cos(Thetan) * dt #change in x
        Delta_Yn = 0.5 * r * (UL + UR) * math.sin(Thetan) * dt# change in y
        Xn += Delta_Xn#add change in x to original and then assign as next x
        Yn += Delta_Yn#add change in y to original and then assign as next y
        Thetan += (r / L) * (UR - UL) * dt #orientation of robot
        D += math.hypot(Delta_Xn, Delta_Yn)#distance of action in 0.1 s
        path_x.append(Xn)#add Xn to path x list
        path_y.append(Yn)#add Yn to path y list

        x_map = int(Xn)
        y_map = int(Yn)
        #if any action goes through obstacle, clearance or out of bounds, return 3 because that will be used to disregard action
    ...
    Thetan = math.degrees(Thetan) % 360 #convert to degrees and change to value from 0 to 359 degree
    return Xn, Yn, Thetan, D, path_x, path_y

def heuristic(x, y, goal):#heuristic is shortest distance from current node to goal node
    return math.hypot(goal[0] - x, goal[1] - y)

# === Node class for A* to initiaize and store parameters useful for backtracking like actions to get to goal node===
class Node:
    def __init__(self, x, y, theta, g, h, parent=None, path_x=None, path_y=None, action=None):
        ...
        self.f = g + h
        self.parent = parent 
        self.path_x = path_x or [] # x coordinates for path
        self.path_y = path_y or []# y coordinates for path
        self.action = action#action refers to Ul and UR

    def __lt__(self, other): #method to check if total cost of one node is less than another, important for deciding priority
        return self.f < other.f

#discretize x,y and theta for visting nodes and ensure there is no duplcation while consuming less time
...
#a star algorith with non holonomic constraints
def astar_with_matrix(start, goal, actions):
    ...
    goal_radius = 0.25*scale#goal radius is 0.25 m
    ...
    #start node is original start point
    start_node = Node(start[0], start[1], start[2], 0, heuristic(start[0], start[1], goal))
    heapq.heappush(open_list, start_node)#add node to open list
    gem = 0
    frame_count = 0#frame counter used later to output live visualization
    start_time = time.time()#start algorithm
    while open_list:
        current = heapq.heappop(open_list)#pop lowest total cost node
        gem += 1

        if heuristic(current.x, current.y, goal) <= goal_radius:#node is in goal range, backtrack and plot and stop time to indicate end of pathfinding
        ...
        #discretize x,y and theta to check if visited node matrix for said values is visited or not
        x_idx, y_idx, theta_idx = state_to_index(current.x, current.y, current.theta)
        if V[x_idx, y_idx, theta_idx]:#if true then continue, else set to true
            continue
        ...
        for action in actions:
            #explore actions, if path goes through obstacle, then go to next action
            ...
            #if out of bounds, skip to next action
            ...
            # If the node was not found in the open list, it's a new node — add it to the open list
            if not found_in_open_list:
                new_node = Node(xn, yn, thetan, g_new, h_new, parent=current, path_x=path_x, path_y=path_y, action=action)
                heapq.heappush(open_list, new_node) #push to queue
                # Visualize this new node in purple
                ax.plot(xn, yn, marker='o', color='purple', markersize=1)  # Frontier node

            #plot trajector of action
            ax.plot(path_x, path_y, color='gray', linewidth=0.4)

        frame_count += 1
        #Doing this to show quick visualization
        if 0<=goal[0] and goal[0]<400:
            if frame_count % 10 == 0:#output every 10th frame
                plt.pause(0.001)
            ...
            if frame_count % 300 == 0:#every 300th frame
                plt.pause(0.001)
