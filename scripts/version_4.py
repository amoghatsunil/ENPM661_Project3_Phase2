...
def point_inside_obstacle(x, y):
    ...
    # Border clearance as semi-algebraic model
    if x < 0.1*scale or x > canvas_width - 0.1*scale or y < scale*0.1 or y > canvas_height - 0.1*scale:
        return True

    # Wall obstacles (half-plane + intervals) using pixel coordinates
...
def clearance_obstacles(clearance):
    ...
    # Mark clearance (dilated area excluding original obstacle) as 2
    clearance_mask[(clearance_mask == 1) & (obstacle_mask == 0)] = 2

    # Ensure obstacle pixels remain 1
